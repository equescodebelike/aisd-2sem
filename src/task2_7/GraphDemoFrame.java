...
import util.graph.GraphvizExamples;
    ...
    public GraphDemoFrame() {
        ...
        buttonCreateGraph.addActionListener(e -> {
            try {
                ...
                Class clz = Class.forName("util.graph." + className);
        ...
        buttonDfsRecursion.addActionListener(e -> {
            try {
                ...
                showSystemOut(() -> {
                    int from = (int) spinnerFromVertex.getValue();
                    System.out.println("Element A: " + from);
                    int to = 5; // до какого выполняем поиск
                    System.out.println("Element B: " + to);
                    LinkedList<Integer> visited = new LinkedList(); // элементы в которых были
                    visited.add(from); // добавляем начальный
                    Task7.findBetween(graph, visited, from, to); // вызов решения
                });
            } catch (Exception exc) {
                SwingUtils.showErrorMessageBox(exc);
            }
